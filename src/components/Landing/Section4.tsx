          ...
          <div className="lg:col-span-7 order-1 lg:order-2 my-auto">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {benefits.map((benefit) => (
                <div
                  key={benefit.id}
                  className="relative group p-4 sm:p-5 bg-white/40 backdrop-blur-sm border border-gray-200/30 rounded-lg sm:rounded-xl transition-all duration-300 transform hover:scale-[1.0] hover:-translate-y-[2px] hover:bg-[linear-gradient(135deg,rgba(147,51,234,0.2),rgba(236,72,153,0.2))] hover:border-[rgba(147,51,234,0.4)] hover:shadow-[0_8px_24px_rgba(147,51,234,0.3)]"
                >
                  {/* Enhanced gradient overlay */}
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-50/20 via-pink-50/20 to-blue-50/20 opacity-0 group-hover:opacity-100 rounded-lg sm:rounded-xl transition-opacity duration-300" />
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start gap-2.5 sm:gap-3">
                      <div
                        className={`w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br ${benefit.bgColor} rounded-lg sm:rounded-xl flex items-center justify-center border ${benefit.borderColor} flex-shrink-0 shadow-sm opacity-60`}
                      >
                        <benefit.Icon
                          className={`w-5 h-5 sm:w-6 sm:h-6 ${benefit.color}`}
                          strokeWidth={1.5}
                        />
                      </div>

                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm sm:text-base font-semibold text-gray-900 mb-1.5 sm:mb-2 leading-tight">
                          {benefit.title}
                        </h3>
                  ...
                  {/* Corner accent */}
                  <div className="absolute top-2.5 sm:top-3 right-2.5 sm:right-3 w-1.5 h-1.5 sm:w-2 sm:h-2 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full opacity-0 group-hover:opacity-60 transition-opacity duration-300" />
                </div>
              ))}
            </div>

            {/* CTA Section */}
            <div className="mt-6 sm:mt-8 p-4 sm:p-6 bg-white/15 backdrop-blur-xl border border-white/20 rounded-xl sm:rounded-2xl text-center shadow-xl">
              <div className="absolute inset-0 bg-gradient-to-br from-purple-50/30 to-pink-50/30 rounded-xl sm:rounded-2xl" />
