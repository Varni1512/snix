...
const InfiniteGallery = () => {
  ...
  const getImageSize = () => {
    switch (screenSize) {
      case 'mobile': return 'w-52 h-72';
      case 'tablet': return 'w-64 h-80';
      default: return 'w-72 h-96';
    }
  };
          ...
          <div className="flex overflow-hidden">
            ...
            >
              ...
              {[...modelImages, ...modelImages, ...modelImages].map((model, idx) => (
                <div
                  key={`${model.id}-${idx}`}
                  className={`${getImageSize()} flex-shrink-0 group cursor-pointer`}
                >
                  <div className="relative h-full bg-white rounded-2xl overflow-hidden shadow-xl transition-all duration-500 border-2 border-transparent">
                    <img
                      src={model.src}
                      alt={model.alt}
                    ...
                    {/* Hover overlay */}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0  transition-opacity duration-400">
                      <div className="absolute bottom-4 left-4 text-white">
                        <h3 className="font-bold text-lg">{model.name}</h3>
                        <p className="text-sm opacity-90">{model.style}</p>
                      </div>
                    </div>
